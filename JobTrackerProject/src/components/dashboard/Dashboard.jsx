...
import { getJobs, deleteJob, createJob, updateJob } from "../../utils/jobApi";
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 mt-4 md:grid-cols-2 lg:grid-cols-3">
          {jobs.map((job) => (
            <div key={job._id} className="bg-white p-4 rounded-xl shadow hover:shadow-md transition">
              ...
              {/* Status dropdown — options match backend enum */}
              <select
                value={job.status}
                onChange={async (e) => {
                ...
                className="border p-1 mt-3 w-full rounded text-sm"
              >
              ...
              {/* Status badge */}
              <span className={`inline-block mt-2 text-xs px-2 py-1 rounded-full font-medium ${statusColors[job.status] || ""}`}>
                {job.status}
              </span>
